...
Un proceso que sigue un *proceso de Poisson* es aquel que tiene un cantidad de eventos promedio cada determinada unidad de medida (generalmente tiempo). Estos sucesos deben ser independientes y aleatorios pero la cantidad debe estar centrada alrededor del promedio ya mencionado. Una última condición es que dos sucesos no pueden suceder en simultáneos.
...
El concepto es el siguiente. Si queremos saber la probabilidad de que haya que esperar al menos X minutos, es lo mismo que calcular la probabilidad de que la cantidad de pacientes en el momento t+x sea igual a la cantidad de momento t. $N_{t+x} = N_{t}$.
...
Este último resultado es la **función de probabilidad acumulada de la exponencial**. La probabilidad de tener que esperar X o menos minutos depende de $\lambda$ and x.
...
Por otro lado podemos ver la probabilidad acumulada hasta determinado minuto. Siguiendo el mismo ejemplo de 10 pacientes por hora (Poisson con $\lambda = 10$), la acumulado de la distribución exponencial tiene la siguiente forma.
