...
* W1 = W2 because we run it twice (two scenarios) and we can re use the same weights because of the gradient calculated
...
y = torch.tensor([1]) # (el ejemplo es de la clase 2)

```

We transform everything to positives to make it cleaner and we add the requires_grad characteristic
...
As we can see in the results, only the weights that go to the correct class' output cell are modified. Classes one and three rest untouched. This is because the sigmoid activation just have the individual weights (and cross entropy only look to the correct class)
```{python}
...
```
On the contrary, when running the same net but with softmax layer we see that all the weights are updated. The correct class have gradient with the same sign that for the sigmoid example but the other two classes have in this case opposite sign gradients (which makes sense since you want them to go in the other direction).  
