...
Vemos que estimando 1000 modelos a partir de 500 observaciones independientes de la población original obtenemos para b0 estimaciones centradas aproximadamente en el valor real (3.915) pero con un mínimo encontrado en 3.306 y un máximo en 4.600. El desvío estándar de la estimación es de 0.210.
...
Vemos que a partir de entrenar el modelo con las 500 observaciones obtenemos un intercepto de 3.751 y un b1 estimado de 5.028.
...
Voilà. Corrimos 10000 iteraciones de nuestro modelo a partir de 10000 muestras de nuestra data original. El desvío estándar de b0 para bootstrap quedó de 0.2102. Que si comparamos con el de 1000 muestras independientes que era 0.2106 es prácticamente igual.
A su vez, podemos calcular el desvío teórico de b0 a partir del modelo (la solución fácil).
```{r}
summary(ml_train_sample)
```

Vemos que desde R el modelo nos devuelve que b0 tiene un desvío de 0.20975. Prácticamente igual al desvío de las muestras independientes como al de bootstrap.
